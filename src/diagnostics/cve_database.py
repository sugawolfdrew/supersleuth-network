...
def _row_to_dict(row: sqlite3.Row) -> Dict[str, Any]:
    ...
    if result.get('references'):
        try:
            result['references'] = json.loads(result['references'])
        except:
            result['references'] = []
...
def _cache_cve(db: CVEDatabase, cve_data: Dict[str, Any]):
    ...
    cursor.execute('''
        INSERT OR REPLACE INTO cves (
            ...
            published_date, last_modified_date, references,
...
def _fetch_cve_from_nvd(cve_id: str) -> Optional[Dict[str, Any]]:
    """
    Fetch CVE data from NVD API.
    
    Note: In production, you would need an API key for NVD API 2.0
    """
    # Simulated CVE data for demonstration
    # In production, make actual API call to https://services.nvd.nist.gov/rest/json/cves/2.0
...
def _search_nvd_by_keyword(keyword: str, limit: int = 50) -> List[Dict[str, Any]]:
    """Search NVD by keyword."""
    # Simulated search - in production, use NVD API
    results = []
    
    # Add some sample results if keyword matches
...
def sync_nvd_feed(year: int = None, api_key: str = None) -> Dict[str, Any]:
    """
    Sync CVE data from NVD feeds.
    
    Args:
        year: Year to sync (default: current year)
        api_key: NVD API key (required for API 2.0)
    ...
    # In production, download and process NVD JSON feeds
